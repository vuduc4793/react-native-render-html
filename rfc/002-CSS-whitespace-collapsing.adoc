...
Given _tnode_ a <<tnode>> and _wrappernode_ a <<tphrasing>> node:

. *Let* _newlist_ be an empty list of <<tnode,TNodes>>.
...
.... *Push* _nextchild_ to _newlist_
... *Else*
.... *Push* _newchild_ to _newlist_
... *End if*
.. *End if*
. *Done*
...
. *Do*: remove sequences of <<collpasible>>, spaces and
tabs immediately preceding or
following a <<segment-break>> *on* _collapsedData_
. *Do*: remove any <<collpasible>> <<segment-break>> immediately following another
<<collpasible>> <<segment-break>> *on* _collapsedData_
