...
isCollapsibleLeft():: Default implementation returns `false`.
isCollapsibleRight():: Default implementation returns `false`.
isWhitespace():: Default implementation returns `false`.
trimLeft():: Default implementation does nothing.
trimRight():: Default implementation does nothing.
...
trimLeft():: Invokes _firstChild.trimLeft_.
trimRight():: Invokes _lastChild.trimRight_.
...
. *For each* _child_ of _node.children_, do:
...
... *End if*
... *Push* the result of applying <<collapse-node>> to _child_ in _node.children_
.. *Endif*
. *Done*
...
[arabic]
. *Let* _newChildren_ be an empty list of <<tnode>>
. *For each* _child_ of _node.children_, do:
.. *If* _child_ is a <<ttext>>
... *Push* the result of applying <<collapse-text>> to _child_ to _newChildren_
.. *Else if* _child_ is a <<tphrasing>>
... *Push* the result of applying <<collapse-phrasing>> to _child_ to _newChildren_
. *Done*
. *Set* _node.children_ to _newChildren_
. *For each* pair index (_i_, _k_) of consecutive items in _node.children_, do:
